/**
 * Página Gerenciar Excursões - lógica externalizada para compatibilidade com CSP.
 */
...
(function() {
  ...
  async function loadExcursoes() {
    ...
    try {
      ...
      console.log('[Excursões Admin] 📦 EXCURSÕES RETORNADAS:', {
        ...
        primeiras3: excursoes.slice(0, 3).map(function(e) { return { id: e.id, titulo: e.titulo }; })
      });
      renderExcursoes(excursoes);
    } catch (error) {
      console.error('[Excursões Admin] ❌ ERRO AO CARREGAR:', error);
      ...
      if (typeof showToast === 'function') showToast('Erro ao carregar excursões: ' + error.message, 'error');
      ...
      if (emptyState) {
        emptyState.innerHTML = '<i class="fas fa-exclamation-circle" style="font-size: 4rem; color: var(--danger-color); margin-bottom: 1rem;"></i>' +
          '<h3 style="color: var(--text-dark);">Erro ao carregar excursões</h3>' +
        ...
        if (btn) btn.addEventListener('click', loadExcursoes);
      }
    }
  }
  ...
  function renderExcursoes(excursoes) {
    ...
    grid.innerHTML = excursoes.map(function(excursao) {
      ...
      return '<div class="excursao-card" data-id="' + escapeHtml(excursao.id) + '" style="border: 2px solid var(--light-border); border-radius: var(--radius-lg); overflow: hidden;">' +
        ...
        '<strong style="color: var(--primary-color); font-size: 1.25rem;">' + (ExcursaoManager.formatPrice ? ExcursaoManager.formatPrice(excursao.preco) : excursao.preco) + '</strong>' +
    ...
    grid.querySelectorAll('.btn-edit-excursao').forEach(function(btn) {
      btn.addEventListener('click', function() { window.location.href = 'excursao-editor.html?id=' + encodeURIComponent(this.getAttribute('data-id')); });
    });
  ...
  async function filterExcursoes() {
    ...
    if (searchTerm) {
      excursoes = excursoes.filter(function(e) {
        return (e.titulo && e.titulo.toLowerCase().indexOf(searchTerm) !== -1) || (e.subtitulo && e.subtitulo.toLowerCase().indexOf(searchTerm) !== -1);
      });
    }
  ...
  function editExcursao(id) {
    window.location.href = 'excursao-editor.html?id=' + encodeURIComponent(id);
  }
  ...
  async function toggleStatus(excursaoId) {
    try {
      var excursao = await ExcursaoManager.getById(excursaoId);
      if (!excursao) {
        if (typeof showToast === 'function') showToast('Excursão não encontrada.', 'error');
        return;
      }
      var newStatus = excursao.status === 'ATIVO' ? 'INATIVO' : 'ATIVO';
      var result = await ExcursaoManager.toggleStatus(excursaoId, newStatus);
      if (result) {
        if (typeof showToast === 'function') showToast('Excursão ' + (newStatus === 'ATIVO' ? 'ativada' : 'desativada') + ' com sucesso!', 'success');
        loadExcursoes();
      } else {
        if (typeof showToast === 'function') showToast('Erro ao alterar status.', 'error');
      }
    } catch (error) {
      console.error('[Excursões Admin] Erro ao alterar status:', error);
      if (typeof showToast === 'function') showToast('Erro ao alterar status.', 'error');
    }
  }
  ...
  function deleteExcursao(excursaoId, titulo) {
    if (typeof confirmDelete === 'function') {
      confirmDelete(titulo, function() {
        ExcursaoManager.delete(excursaoId).then(function(success) {
          if (success) {
            ...
            if (card) {
              ...
              setTimeout(loadExcursoes, 300);
            }
          } else {
            if (typeof showToast === 'function') showToast('Erro ao excluir excursão.', 'error');
          }
        }).catch(function(err) {
          console.error('[Excursões Admin] Erro ao excluir:', err);
          if (typeof showToast === 'function') showToast('Erro ao excluir excursão.', 'error');
        });
      });
    }
  ...
  // Expõe para chamadas globais (ex.: HTML que ainda use onclick em outros contextos)
  window.loadExcursoes = loadExcursoes;
  window.filterExcursoes = filterExcursoes;
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var searchExcursoes = document.getElementById('searchExcursoes');
    if (searchExcursoes) searchExcursoes.addEventListener('keyup', filterExcursoes);
    var filterCategoria = document.getElementById('filterCategoria');
    if (filterCategoria) filterCategoria.addEventListener('change', filterExcursoes);
    var filterStatus = document.getElementById('filterStatus');
    if (filterStatus) filterStatus.addEventListener('change', filterExcursoes);
  });
})();
