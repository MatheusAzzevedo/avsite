/**
 * Editor de Excursões Pedagógicas - Gerenciar criação e edição
 */

// Variáveis globais
let isEditing = false;
let currentExcursaoId = '';
let galeriaImages = [];
...
function showNotification(message, type = 'info') {
  ...
  console.warn('[Excursão Pedagógica Editor] showToast não disponível. Mensagem:', message);
  alert(message);
}
...
/**
 * Carrega dados da excursão pedagógica para edição
 */
async function loadExcursao(excursaoId) {
  try {
    console.log('[Excursão Pedagógica Editor] Carregando excursão pedagógica para edição:', excursaoId);

    const excursao = await ExcursaoPedagogicaManager.getById(excursaoId);

    if (!excursao) {
      showNotification('Excursão pedagógica não encontrada!', 'error');
      setTimeout(() => (window.location.href = 'excursoes-pedagogicas.html'), 2000);
      return;
    }
    ...
    console.log('[Excursão Pedagógica Editor] Excursão pedagógica carregada com sucesso');
  } catch (error) {
    console.error('[Excursão Pedagógica Editor] Erro ao carregar:', error);
    showNotification('Erro ao carregar excursão pedagógica!', 'error');
  }
}
...
/**
 * Valida os dados obrigatórios da excursão pedagógica
 */
function validateExcursao(excursaoData) {
  // Valida código
  if (!excursaoData.codigo) {
    showNotification('O código é obrigatório.', 'error');
    document.getElementById('excursaoCodigo').focus();
    return false;
  }

  // Valida formato do código (apenas letras, números, hífen e underscore)
  const codigoRegex = /^[A-Za-z0-9_-]+$/;
  if (!codigoRegex.test(excursaoData.codigo)) {
    showNotification('O código deve conter apenas letras, números, hífen e underscore.', 'error');
    document.getElementById('excursaoCodigo').focus();
    return false;
  }
...
/**
 * Salva a excursão pedagógica via API (cria nova ou atualiza existente)
 */
async function saveExcursao(event) {
  ...
  try {
    console.log('[Excursão Pedagógica Editor] Salvando excursão pedagógica:', excursaoData);
    let result;

    if (isEditing && currentExcursaoId) {
      result = await ExcursaoPedagogicaManager.update(currentExcursaoId, excursaoData);

      if (result) {
        showNotification('Excursão pedagógica atualizada com sucesso!', 'success');
      } else {
        showNotification('Erro ao atualizar excursão pedagógica.', 'error');
        return;
      }
    } else {
      result = await ExcursaoPedagogicaManager.create(excursaoData);

      if (result) {
        showNotification('Excursão pedagógica criada com sucesso!', 'success');
      } else {
        showNotification('Erro ao criar excursão pedagógica.', 'error');
        return;
      }
    }

    console.log('[Excursão Pedagógica Editor] Excursão pedagógica salva:', result);
    ...
    showNotification(error.message || 'Erro ao salvar excursão pedagógica.', 'error');
  } finally {
...
/**
 * Salva a excursão pedagógica como inativa
 */
